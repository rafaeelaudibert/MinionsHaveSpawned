...
namespace Constants {
    ...
    const float SPAWN_RATE = 1.35f;                                                 // How many seconds between each minion spawn
    const float START_GOLD = 150.0f;                                                // Game gold start quantity
    const float GOLD_PER_WAVE = 100.0f;                                             // Gold earned each time a wave finishes
    ...
    const float SIDE_LANE_SPEED_MULTIPLIER = 1.6f;                                  // Minions on the side lane walk this speed faster
