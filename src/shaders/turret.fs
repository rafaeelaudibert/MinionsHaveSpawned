...
// Check if we want to render it in red shadow or not
uniform bool can_place;

// Check if we want to render it with alpha or not
uniform bool placed;
...
void main()
{
    ...
    // If cannot place, change the lights to RED
    if (!can_place) {
        I = vec4(2.0, 0.0, 0.0, 1.0);
        Ia = vec4(0.4, 0.0, 0.0, 1.0);
    }
